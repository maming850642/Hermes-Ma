...
Hermes-Ma 是一个支持多用户使用的agent，并且通过按钮切换，允许把"1 个被动聊天 agent"切换成"**半自主的 AI 智囊团**"：常驻一个总参（chief-of-staff），按需 fork 多个真员工 worker，通过**文件总线**协作，专把模糊需求拆解、围攻、整合，产出"你能拍板的具体决策清单"。用户只做最不可替代的拍板，最耗心智的"想清楚到底做什么"由智囊团承担。
